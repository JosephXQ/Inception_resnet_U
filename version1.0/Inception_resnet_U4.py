# -*- coding: utf-8 -*-
"""
Created on Thu Jun 28 16:27:56 2018
...
class DnCNN(object):
    def __init__(self, sess, image_size=128, batch_size=48, decay_epoch=30,
                 ...
                 dataset='trainmodel_U4+Inception-resnet_60-1200_16b_nmumse', testset='test'):
        self.sess = sess
    ...
    def build_model(self):
        ...
        with tf.variable_scope('layer3'):
            layer_3_output = self.max_pool(layer_2_output)
        with tf.variable_scope('add1'):
            layer_add1_output = self.bn_conv(layer_3_output,[3,3,64,128])
        with tf.variable_scope('layer4'):
            layer_4_output = self.resblock_layer(layer_add1_output, [3, 3, 128, 128], first_block=True)
        with tf.variable_scope('add2'):
            layer_add2_output = self.max_pool(layer_4_output)
        with tf.variable_scope('add3'):
            layer_add3_output = self.bn_conv(layer_add2_output,[3,3,128,256])
        with tf.variable_scope('layer5'):
            layer_5_output = self.resblock_layer(layer_add3_output, [3, 3, 256, 256])  # +layer_2_output_temp1
        with tf.variable_scope('layer6'):
            layer_6_output = self.max_pool(layer_5_output)
        with tf.variable_scope('layer7'):
            layer_7_output = self.bn_conv(layer_6_output,[3,3,256,512])
        with tf.variable_scope('layer8'):
            layer_8_output = self.Inception_resnetA_layer(layer_7_output, [3, 3, 512, 512])  # +layer_2_output_temp2
        with tf.variable_scope('layer9'):
            layer_9_output = self.max_pool(layer_8_output)
        with tf.variable_scope('layer10'):
            layer_10_output = self.bn_conv(layer_9_output,[3,3,512,1024])
        with tf.variable_scope('layer11'):
            layer_11_output = self.Inception_resnetB_layer(layer_10_output, [3, 3, 1024, 1024])
        with tf.variable_scope('add4'):
            layer_add4_output = self.bn_conv(layer_11_output,[3,3,1024,512])
        with tf.variable_scope('layer12'):
            layer_12_temp = self.unpool(layer_add4_output)
            layer_12_output = tf.concat([layer_12_temp,layer_8_output],3,name = 'concat')
        with tf.variable_scope('layer13'):
            layer_13_output = self.bn_conv(layer_12_output,[3,3,1024,256])
        with tf.variable_scope('layer14'):
            layer_14_output = self.Inception_resnetC_layer(layer_13_output, [3, 3, 256, 256])
        with tf.variable_scope('layer15'):
            layer_15_temp = self.unpool(layer_14_output)
            layer_15_output = tf.concat([layer_15_temp, layer_5_output], 3, name='concat')#mark
        with tf.variable_scope('layer16'):
            layer_16_output = self.bn_conv(layer_15_output, [3, 3, 512, 128])
        with tf.variable_scope('layer17'):
            layer_17_output = self.resblock_layer(layer_16_output, [3, 3, 128, 128])
        with tf.variable_scope('add5'):
            layer_add5_temp = self.unpool(layer_17_output)
            layer_add5_output = tf.concat([layer_add5_temp,layer_4_output],3,name='concat')
        with tf.variable_scope('add6'):
            layer_add6_output = self.bn_conv(layer_add5_output,[3,3,256,64])
        with tf.variable_scope('layer18'):
            layer_18_output = self.resblock_layer(layer_add6_output, [3, 3, 64, 64])
        with tf.variable_scope('add7'):
            layer_add7_temp = self.unpool(layer_18_output)
            layer_add7_output = tf.concat([layer_add7_temp,layer_2_output],3,name='concat')
        with tf.variable_scope('add8'):
            layer_add8_output = self.bn_conv(layer_add7_output,[3,3,128,64])
        with tf.variable_scope('layer19'):
            self.Y = self.bn_relu_conv_layer(layer_add8_output, [3, 3, 64, self.output_c_dim])
        # MSE loss
    ...
    def Inception_in_resnetA_layer(self, inputdata, filter_shape, b_init=0.0, stridemode=[1, 1, 1, 1], useBN=True):
        with tf.variable_scope('conv1_in_Inception_in_A'):
            input1_data = self.conv_layer(inputdata, [1, 1, filter_shape[-2], 256], b_init, stridemode)
        ...
        with tf.variable_scope('conv3_in_Inception_in_A'):
            input3_data1 = self.conv_layer(inputdata, [1, 1, filter_shape[-2], 128], b_init, stridemode)
            with tf.variable_scope('subconv1_in_Inconv3'):
                input3_data2 = self.bn_relu_conv_layer(input3_data1, [3, 3, 128, 128], b_init, stridemode)
                with tf.variable_scope('subconv2_in_Inconv3'):
                    input3_data = self.bn_relu_conv_layer(input3_data2, [3, 3, 128, 128], b_init, stridemode)
        output = tf.concat([input1_data, input2_data, input3_data], 3, name='concat')
        return output
    ...
    def Inception_in_resnetB_layer(self, inputdata, filter_shape, b_init=0.0, stridemode=[1, 1, 1, 1]):
        with tf.variable_scope('conv1_in_Inception_in_B'):
            input1_data = self.conv_layer(inputdata, [1, 1, filter_shape[-2], 512], b_init, stridemode)
        with tf.variable_scope('conv2_in_Inception_in_B'):
            input2_data1 = self.conv_layer(inputdata, [1, 1, filter_shape[-2], 512], b_init, stridemode)
            with tf.variable_scope('subconv1_in_Inconv2'):
                input2_data2 = self.bn_relu_conv_layer(input2_data1, [1, 3, 512, 512], b_init, stridemode)
                with tf.variable_scope('subconv2_in_Inconv2'):
                    input2_data = self.bn_relu_conv_layer(input2_data2, [3, 1, 512, 512], b_init, stridemode)
    ...
    def Inception_in_resnetC_layer(self, inputdata, filter_shape, b_init=0.0, stridemode=[1, 1, 1, 1]):
        ...
        with tf.variable_scope('conv2_in_Inception_in_C'):
            input2_data1 = self.conv_layer(inputdata, [1, 1, filter_shape[-2], 128], b_init, stridemode)
            with tf.variable_scope('subconv1_in_Inconv2'):
                input2_data2 = self.bn_relu_conv_layer(input2_data1, [1, 7, 128, 128], b_init, stridemode)
                with tf.variable_scope('subconv2_in_Inconv2'):
                    input2_data = self.bn_relu_conv_layer(input2_data2, [7, 1, 128, 128], b_init, stridemode)
        output = tf.concat([input2_data, input1_data], 3, name='concat')
        return output
    ...
    def sampler(self, image):
        ...
        with tf.variable_scope('layer3',reuse=True):
            layer_3_output = self.max_pool(layer_2_output)
        with tf.variable_scope('add1',reuse=True):
            layer_add1_output = self.bn_conv(layer_3_output,[3,3,64,128])
        with tf.variable_scope('layer4',reuse=True):
            layer_4_output = self.resblock_layer(layer_add1_output, [3, 3, 128, 128], first_block=True)
        with tf.variable_scope('add2',reuse=True):
            layer_add2_output = self.max_pool(layer_4_output)
        with tf.variable_scope('add3',reuse=True):
            layer_add3_output = self.bn_conv(layer_add2_output,[3,3,128,256])
        ...
        with tf.variable_scope('layer11',reuse=True):
            layer_11_output = self.Inception_resnetB_layer(layer_10_output, [3, 3, 1024, 1024])
        with tf.variable_scope('add4',reuse=True):
            layer_add4_output = self.bn_conv(layer_11_output,[3,3,1024,512])
        ...
        with tf.variable_scope('layer17',reuse=True):
            layer_17_output = self.resblock_layer(layer_16_output, [3, 3, 128, 128])
        with tf.variable_scope('add5',reuse=True):
            layer_add5_temp = self.unpool(layer_17_output)
            layer_add5_output = tf.concat([layer_add5_temp,layer_4_output],3,name='concat')
        with tf.variable_scope('add6',reuse=True):
            layer_add6_output = self.bn_conv(layer_add5_output,[3,3,256,64])
        with tf.variable_scope('layer18',reuse=True):
            layer_18_output = self.resblock_layer(layer_add6_output, [3, 3, 64, 64])
        with tf.variable_scope('add7',reuse=True):
            layer_add7_temp = self.unpool(layer_18_output)
            layer_add7_output = tf.concat([layer_add7_temp,layer_2_output],3,name='concat')
        with tf.variable_scope('add8',reuse=True):
            layer_add8_output = self.bn_conv(layer_add7_output,[3,3,128,64])
        with tf.variable_scope('layer19',reuse=True):
            self.Y_test = self.bn_relu_conv_layer(layer_add8_output, [3, 3, 64, self.output_c_dim])
            print("layer last........")
